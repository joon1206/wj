...
// An aligned malloc. Use afree() or free() to free. Only works for unix
__attribute__((__always_inline__, 
               __malloc__, __alloc_size__(1),
               __alloc_align__(2)))
static __inline__ void *amalloc(size_t __size, size_t __align)
{
        if (__align == 1)
...
// The corresponding free to amalloc.
__attribute__((__always_inline__))
static __inline void afree(void *__mallocedMemory)
{
        free(__mallocedMemory);
}
#endif
